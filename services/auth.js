 ...
 * @param {string} phone_mobile : phone_mobile of user.
 ...
 */
const loginUser = async (phone_mobile,password,platform,roleAccess) => {
  try {
    let where = { 'phone_mobile':phone_mobile };
    ...
    if (user) {
      ...
      if (platform == PLATFORM.DAVIVIENDA){
        if (!LOGIN_ACCESS[user.userType].includes(PLATFORM.DAVIVIENDA)){
          return {
            flag:true,
            data:'you are unable to access this platform'
          };
        }
        token = await generateToken(userData,JWT.DAVIVIENDA_SECRET);
      }
  ...
  try {
    ...
    if (FORGOT_PASSWORD_WITH.LINK.email){
      ...
      let mailObj = {
        ...
        data:{ phone_mobile:updatedUser.phone_mobile, }
      };
      try {
