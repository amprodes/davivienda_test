...
const register = async (req,res) =>{
  try {
    ...
    let checkUniqueFields = await common.checkUniqueFieldsInDatabase(User,[ 'phone_mobile' ],data,'INSERT');
...
/**
 * @description : login with phone_mobile and password
...
const login = async (req,res)=>{
  try {
    ...
    if (!phone_mobile || !password){
      return res.badRequest({ message : 'Insufficient request parameters! phone_mobile and password is required.' });
    }
    let roleAccess = false;
    if (req.body.includeRoleAccess){
      roleAccess = req.body.includeRoleAccess;
    }
    let result = await authService.loginUser(phone_mobile, password, authConstant.PLATFORM.DAVIVIENDA, roleAccess);
    if (result.flag){
...
const forgotPassword = async (req,res) => {
  ...
  try {
    ...
    // }
    if (!found) {
      return res.status(404).json({ status: "RECORD_NOT_FOUND", message: "User record not found" });
    }
    let {
    ...
    if (resultOfEmail && resultOfSMS){
      return res.success({ status: "SUCCESS", message :'otp successfully send.' });
    } else if (resultOfEmail && !resultOfSMS) {
      return res.success({ status: "SUCCESS", message : 'otp successfully send to your email.' });
    } else if (!resultOfEmail && resultOfSMS) { 
      return res.success({ status: "SUCCESS", message : 'otp successfully send to your mobile number.' });
    } else {
